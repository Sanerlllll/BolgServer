...
export default {
  ...
  data() {
    ...
    //  表单验证规则
    // el-form-item元素的prop属性绑定字段名，表单验证时，就会验证el-input元素绑定的变量loginRules.字段名的值是否符合验证规则
    loginRules:{
      // 各个字段的验证规则,每个字段名与prop关联
      loginId:[{
        required:true,
        trigger:'blur',//什么时候触发
        message:'请输入管理员账号'
      ...
      // {
      //   min:6,max:15,message:'长度需要在6-15字符之间'
      // },{
      //   validator:checkPassword,trigger:'blur'}//checkPassword方法名
      ],
      captcha:[{
        required:true,
        trigger:'blur',//什么时候触发
